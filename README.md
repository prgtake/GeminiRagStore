...
### EXE ファイルの作成方法 (Windows)

自分で実行ファイル（EXE）を作成したい場合は、リポジトリに含まれる `build.bat` をダブルクリックして実行してください。
完了すると、`dist` フォルダの中に `GeminiRagStore.exe` が生成されます。

※ 実行には Python 3.8 以上が必要です。
